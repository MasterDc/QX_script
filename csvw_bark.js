...
# 通知 = Bark(iOS推送,已内置设备key,默认直接可用;覆盖设环境变量):
# BARK_KEY=Bark设备key(api.day.app后面那串)
# BARK_URL=完整Bark地址(自建服务器可填自己的,默认官方 https://api.day.app/KEY)
# 签到完成后推送标题"上汽大众签到",内容为签到结果
...
!(async () => {
    ...
    .finally(async () => {
        const _msg = $.messages.join('\n').trimStart().trimEnd();
        await sendMsg(_msg);  // 推送通知(青龙 sendNotify / QX 弹窗)
        try { await barkNotify(_msg); } catch (e) { $.log('Bark通知异常: ' + (e.message || e)); }
        $.done();
    })
...
// ============ Bark 通知模块 ============
// 已内置 Bark 设备 key,不配置任何环境变量也会推送(iOS 收到通知)。
// 覆盖方式(环境变量优先级高于内置默认值):
//   BARK_KEY = Bark 设备 key(api.day.app 后面那串,自建服务器同理)
//   BARK_URL = 完整 Bark 地址,形如 https://api.day.app/KEY 或自建 https://bark.xxx.com/KEY
// Bark 无需关键词/加签,key 泄露风险仅为被刷通知,可放心放公开仓库
const BARK_KEY_DEFAULT = '3WXNuFpH9SWdxRFxiX9bDf';
const BARK_URL_DEFAULT = 'https://api.day.app/';
const BARK_KEY = getEnv('bark_key') || BARK_KEY_DEFAULT;
const BARK_URL = getEnv('bark_url') || '';

async function barkNotify(text) {
    if (!text) return;
    let base = BARK_URL || (BARK_URL_DEFAULT + BARK_KEY);
    base = base.replace(/\/+$/, ''); // 去掉结尾斜杠,避免 //Title
    const title = '上汽大众签到';
    // Bark 路径参数需 URL 编码;group 用于把通知收进同一分组
    const url = base + '/' + encodeURIComponent(title) + '/' + encodeURIComponent(text) + '?group=csvw';
    const r = await httpRequest(url, 'Bark通知');
    const ok = r && (r.code === 200 || r.code === '200');
    $.log(ok ? '✅Bark推送成功' : ('❌Bark推送失败: ' + $.toStr(r)));
}
